    ...
    <div class="panel-body">
        <div class="container-fluid">
            <div class="row justify-content-center">
                <div class="col-md-10">
                    <form action="{{ route('editregulations.update', $regulations->id ?? 0) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <div class="mb-3">
                            <label for="content" class="form-label">規範內容</label>
                            <textarea id="content" name="content" class="form-control" rows="23" required>{{ old('content', $regulations->content ?? '') }}</textarea>
                        </div>

                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">儲存更新</button>
                            <a href="{{ url('/residentsregulations') }}" class="btn btn-secondary">取消</a>
                        </div>
                    </form>
                </div>
            </div>
...
<div class="corner-dock" aria-label="右下角功能列">
    <a class="dock-btn is-active" href="{{ url('/editregulations') }}" style="text-decoration: none;"><i class="bi bi-pencil-square"></i> 編輯規章</a>
    <a class="dock-btn " href="{{ url('/residentsregulations') }}" style="text-decoration: none;"><i class="bi bi-journal-medical"></i> 規章內容</a>
</div>
