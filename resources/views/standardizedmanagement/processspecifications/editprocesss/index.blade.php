    ...
    <div class="panel-body">
        <div class="container-fluid">
            <div class="row justify-content-center">
                <div class="col-md-10">
                    <form action="{{ route('editprocesss.update', $process->id ?? 0) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <div class="mb-3">
                            <label for="content" class="form-label">規範內容</label>
                            <textarea id="content" name="content" class="form-control" rows="23" required>{{ old('content', $process->content ?? '') }}</textarea>
                        </div>

                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">儲存更新</button>
                            <a href="{{ url('/processspecifications') }}" class="btn btn-secondary">取消</a>
                        </div>
                    </form>
                </div>
            </div>
...
<nav class="quick-toolbar" aria-label="快速工具列">
    ...
    <a class="floor-btn d-flex flex-column align-items-center justify-content-center gap-1 is-active" href="{{ url('/processspecifications') }}">
    ...
    <a class="floor-btn d-flex flex-column align-items-center justify-content-center gap-1 " href="{{ url('/residentsregulations') }}" >
...
<div class="corner-dock" aria-label="右下角功能列">
    <a class="dock-btn is-active" href="{{ url('/editprocesss') }}" style="text-decoration: none;"><i class="bi bi-pencil-square"></i> 編輯規範</a>
    <a class="dock-btn " href="{{ url('/processspecifications') }}" style="text-decoration: none;"><i class="bi bi-journal-text"></i> 規範內容</a>
</div>
