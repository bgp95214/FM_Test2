...
<section class="ba-panel" aria-label="裝置清單">
    ...
    <div class="panel-body">
        ...
        <form action="{{ isset($conference) ? url('/writeconference/' . $conference->id) : url('/writeconference') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if(isset($conference))
                @method('PUT')
            @endif
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $conference->title ?? '') }}" required maxlength="150">
                @error('title')
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="content" name="content" rows="6" required>{{ old('content', $conference->content ?? '') }}</textarea>
                @error('content')
                    <div class="text-danger small mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <div class="form-text">上傳的檔案會儲存到 public/uploads/meeting/conference</div>
                @if(isset($conference) && $conference->photo)
                    <div class="mt-2">
                        <small class="text-muted">目前照片：</small><br>
                        <img src="{{ asset($conference->photo) }}" alt="大會照片" style="max-width: 150px; max-height: 100px; margin-top: 0.5rem;">
                    </div>
                @endif
                @error('photo')
                    <div class="text-danger small mt-1">{{ $message }}</div>
            ...
            <div class="d-flex gap-2 mb-4">
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-check-circle me-1"></i> {{ isset($conference) ? '更新大會資料' : '送出大會資料' }}
                </button>
                @if(isset($conference))
                    <a href="{{ url('/writeconference') }}" class="btn btn-secondary">
...
<div class="corner-dock" aria-label="右下角功能列">
    ...
    <a class="dock-btn is-active" href="{{ url('/writeconference') }}" style="text-decoration: none;"><i class="bi bi-pen-fill"></i> 大會填寫</a>
