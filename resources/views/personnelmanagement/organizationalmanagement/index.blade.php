            ...
            <table class="table table-hover">
                <thead class="table-light">
                    <tr>
                        ...
                        <th scope="col">樓層-門號</th>
                        <th scope="col" style="width: 120px" class="text-center">狀態</th>
                    </tr>
                </thead>
                <tbody>
                @forelse($positions ?? [] as $position)
                    @php
                        $allocation = ($allocations ?? collect())->get($position->id);
                        $resident = $allocation->resident ?? null;
                    @endphp
                    <tr>
                        <td><strong>{{ $position->name }}</strong></td>
                        <td>{{ $resident->name ?? '未分配' }}</td>
                        <td>
                            @if($resident)
                                {{ $resident->floor->name ?? '-' }}-{{ $resident->door_number }}
                            @else
                                -
                            @endif
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="text-center text-muted">尚無職位資料，請先新增職位。</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
...
<div class="corner-dock" aria-label="右下角功能列">
    ...
    <a class="dock-btn is-active" href="{{ url('/organizationalmanagement') }}" style="text-decoration: none;"><i class="bi bi-diagram-3"></i> 組織架構</a>
