...
<section class="ba-panel" aria-label="裝置清單">
    ...
    <div class="panel-body">
        ...
        <form action="@if(isset($position)) {{ url('/addpositions/'.$position->id) }} @else {{ url('/addpositions') }} @endif" method="POST">
            @csrf
            @if(isset($position))
                @method('PUT')
            @endif
            ...
            <div class="row g-3">
                <div class="col-12">
                    <label for="name" class="form-label">職位名稱 <span class="text-danger">*</span></label>
                    <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $position->name ?? '') }}" placeholder="例如：主任、副理、經理" required>
                    @error('name')
                ...
                <div class="col-12">
                    <label for="content" class="form-label">職位負責內容 <span class="text-danger">*</span></label>
                    <textarea class="form-control" id="content" name="content" rows="6" placeholder="請詳細說明該職位的主要職責和工作內容..." required>{{ old('content', $position->content ?? '') }}</textarea>
                    @error('content')
                        <div class="text-danger small mt-1">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mt-4 d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    ...
                    @if(isset($position)) 更新職位 @else 新增職位 @endif
                </button>
                <a href="{{ url('/addpositions') }}" class="btn btn-secondary">
...
<div class="corner-dock" aria-label="右下角功能列">
    <a class="dock-btn is-active" href="{{ url('/addpositions') }}" style="text-decoration: none;">
        <i class="bi bi-{{ isset($position) ? 'pencil-square' : 'database-add' }}"></i> 
        {{ isset($position) ? '編輯職位' : '新增職位' }}
    </a>
