...
<section class="ba-panel" aria-label="裝置清單">
    ...
    <div class="panel-body">
        <h6 class="mb-3" style="color:#334155">大會紀錄清單</h6>
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                @forelse($conferences ?? [] as $conference)
                    <tr>
                        <td>{{ $conference->created_at->format('Y/m/d H:i') }}</td>
                        <td>{{ $conference->title }}</td>
                        <td class="text-center">
                            <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#contentModal{{ $conference->id }}">
                                <i class="bi bi-file-text"></i> 內容
                            </button>
                            @if($conference->photo)
                                <button class="btn btn-sm btn-outline-info" data-bs-toggle="modal" data-bs-target="#photoModal{{ $conference->id }}">
                                    <i class="bi bi-image"></i> 照片
                                </button>
                            @endif
                            <a href="{{ url('/writeconference/' . $conference->id . '/edit') }}" class="btn btn-sm btn-outline-warning">
                                <i class="bi bi-pencil"></i> 編輯
                            </a>
                            <form method="POST" action="{{ url('/writeconference/' . $conference->id) }}" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('確定刪除嗎？')">
                    ...
                    <!-- 內容彈出視窗 -->
                    <div class="modal fade" id="contentModal{{ $conference->id }}" data-bs-backdrop="false" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title">{{ $conference->title }}</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <p style="white-space: pre-wrap;">{{ $conference->content }}</p>
                                </div>
                                <div class="modal-footer">
                                    <small class="text-muted">{{ $conference->created_at->format('Y年m月d日 H:i') }}</small>
                    ...
                    <!-- 照片彈出視窗 -->
                    @if($conference->photo)
                        <div class="modal fade" id="photoModal{{ $conference->id }}" data-bs-backdrop="false" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body text-center">
                                        <img src="{{ asset($conference->photo) }}" alt="大會照片" class="img-fluid" style="max-height: 500px;">
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">關閉</button>
                                    </div>
                ...
                @empty
                    <tr>
                        <td colspan="3" class="text-center text-muted">尚無大會紀錄</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
...
<div class="corner-dock" aria-label="右下角功能列">
    ...
    <a class="dock-btn is-active" href="{{ url('/conference') }}" style="text-decoration: none;"><i class="bi bi-pass-fill"></i> 大會紀錄</a>
