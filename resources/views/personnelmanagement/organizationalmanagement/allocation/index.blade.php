...
<section class="ba-panel" aria-label="裝置清單">
    ...
    <div class="panel-body">
        ...
        <h6 class="mb-3" style="color:#334155">職位分配</h6>
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="card border-0 shadow-sm">
                        <div class="card-body">
                            ...
                            <p class="card-text small text-muted">{{ Str::limit($position->content, 60) }}</p>
                            
                            @php
                                $currentAllocation = collect($allocations ?? [])->firstWhere('position_id', $position->id);
                            @endphp

                            @if($currentAllocation)
                                <div class="alert alert-info p-2 mb-3" role="alert">
                                    <small><strong>目前分配：</strong> {{ $currentAllocation->resident->name }}</small>
                                </div>
                                <form action="{{ url('/allocation/'.$currentAllocation->id) }}" method="POST" class="d-inline" onsubmit="return confirm('確定取消此分配嗎？');">
                                    @csrf
                                    @method('DELETE')
                            ...
                            @else
                                <form action="{{ url('/allocation') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="position_id" value="{{ $position->id }}">
                                    
                                    <div class="mb-2">
                                        <select class="form-select form-select-sm" name="resident_id" required onchange="this.form.submit()">
                                            <option value="">選擇住戶...</option>
                                            @foreach($residents ?? [] as $resident)
                                                <option value="{{ $resident->id }}">
                                                    {{ $resident->name }} ({{ $resident->floor->name ?? '-' }}-{{ $resident->door_number }})
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
...
<div class="corner-dock" aria-label="右下角功能列">
    ...
    <a class="dock-btn is-active" href="{{ url('/allocation') }}" style="text-decoration: none;"><i class="bi bi-database-gear"></i> 職位分配</a>
