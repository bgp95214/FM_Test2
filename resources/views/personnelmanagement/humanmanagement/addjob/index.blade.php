    ...
    <div class="panel-body">
        ...
        <div class="card mb-4">
            <div class="card-header" style="background:#ECECEC">
                <h6 class="mb-0">{{ isset($job) ? '編輯職位' : '新增職位' }}</h6>
            </div>
            <div class="card-body">
                <form action="{{ isset($job) ? route('addjob.update', $job->id) : route('addjob.store') }}" method="POST">
                    @csrf
                    @if(isset($job))
                        @method('PUT')
                    @endif
                    ...
                    <div class="mb-3">
                        <label for="jobName" class="form-label">職稱</label>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="jobName" name="name" 
                               value="{{ $job->name ?? old('name') }}" required>
                        @error('name')
                    ...
                    <div class="mb-3">
                        <label for="jobContent" class="form-label">職稱內容</label>
                        <textarea class="form-control @error('content') is-invalid @enderror" id="jobContent" name="content" 
                                  rows="4">{{ $job->content ?? old('content') }}</textarea>
                        @error('content')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="d-flex gap-2">
                        ...
                        @if(isset($job))
                            <a href="{{ url('/addjob') }}" class="btn btn-secondary">取消</a>
                        @endif
                    </div>
                </form>
            </div>
...
<div class="corner-dock" aria-label="右下角功能列">
    <a class="dock-btn is-active" href="{{ url('/addjob') }}" style="text-decoration: none;"><i class="bi bi-file-earmark-plus"></i> 新增職稱</a>
