    ...
    <div class="panel-body">
        <div class="container-fluid">
            <div class="row justify-content-center">
                <div class="col-md-8">
                    <h4 class="mb-4">編輯人員資料</h4>
                    <form action="{{ route('addhuman.update', $human->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        
                        <div class="mb-3">
                            <label for="name" class="form-label">人員名稱</label>
                            <input type="text" class="form-control" id="name" name="name" value="{{ $human->name }}" required>
                        </div>

                        <div class="mb-3">
                            <label for="job_title" class="form-label">職稱</label>
                            <select class="form-select" id="job_title" name="job_title" required>
                                <option value="">請選擇職稱</option>
                                @foreach($jobs as $job)
                                    <option value="{{ $job->name }}" {{ $human->job_title === $job->name ? 'selected' : '' }}>{{ $job->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="hire_date" name="hire_date" value="{{ $human->hire_date }}" required>
                        </div>

                        <div class="mb-3">
                            <label for="phone" class="form-label">聯絡電話</label>
                            <input type="tel" class="form-control" id="phone" name="phone" value="{{ $human->phone }}" required>
                        </div>

                        <div class="mb-3">
                            <label for="email" class="form-label">Email</label>
                            <input type="email" class="form-control" id="email" name="email" value="{{ $human->email }}" required>
                        </div>

                        <div class="d-grid gap-2 d-sm-flex justify-content-sm-center">
                            <button type="submit" class="btn btn-primary">更新人員</button>
                            <a href="{{ url('/humanmanagement') }}" class="btn btn-secondary">取消</a>
                        </div>
                    </form>
                </div>
            </div>
...
<nav class="quick-toolbar" aria-label="快速工具列">
    ...
    <a class="floor-btn d-flex flex-column align-items-center justify-content-center gap-1 is-active" href="{{ url('/humanmanagement') }}" >
...
<div class="corner-dock" aria-label="右下角功能列">
    <a class="dock-btn " href="{{ url('/addjob') }}" style="text-decoration: none;"><i class="bi bi-file-earmark-plus"></i> 新增職稱</a>
    <a class="dock-btn " href="{{ url('/job') }}" style="text-decoration: none;"><i class="bi bi-file-person"></i> 職稱列表</a>
    <a class="dock-btn" href="{{ url('/addhuman') }}" style="text-decoration: none;"><i class="bi bi-person-add"></i> 新增人員</a>
    <a class="dock-btn" href="{{ url('/humanmanagement') }}" style="text-decoration: none;"><i class="bi bi-person-fill-gear"></i> 人員列表</a>
