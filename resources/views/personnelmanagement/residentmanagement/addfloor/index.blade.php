...
<section class="ba-panel" aria-label="裝置清單">
    ...
    <div class="panel-body">
        ...
        <form action="@if(isset($floor)) {{ url('/addfloor/'.$floor->id) }} @else {{ url('/addfloor') }} @endif" method="POST">
            @csrf
            @if(isset($floor))
                @method('PUT')
            @endif
        ...
        <div class="container-fluid">
            <div class="row justify-content-center">
                <div class="col-lg-8">
                    ...
                    <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $floor->name ?? '') }}" placeholder="例如：1F、2F、B1 等" required>
                    @error('name')
                    ...
                    <div class="mt-4 d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            ...
                            @if(isset($floor)) 更新分類 @else 新增分類 @endif
                        </button>
                        <a href="{{ url('/floors') }}" class="btn btn-secondary">
...
<div class="corner-dock" aria-label="右下角功能列">
    <a class="dock-btn is-active" href="{{ url('/addassets') }}" style="text-decoration: none;">
        <i class="bi bi-{{ isset($floor) ? 'pencil-square' : 'bi bi-stack' }}"></i> 
        {{ isset($floor) ? '編輯樓層' : '新增樓層' }}
    </a>
