...
<section class="ba-panel" aria-label="裝置清單">
    ...
    <div class="panel-body">
        ...
        <div id="residentsWrap" class="table-responsive">
            <table class="table table-hover">
                <thead class="table-light">
                    <tr>
                        <th scope="col">住戶名稱</th>
                        <th scope="col">樓層</th>
                        <th scope="col">門號</th>
                ...
                @forelse($residents ?? [] as $resident)
                    <tr>
                        <td>{{ $resident->name }}</td>
                        <td>{{ $resident->floor->name ?? '-' }}</td>
                        <td>{{ $resident->door_number }}</td>
                        <td>{{ $resident->contact_phone }}</td>
                        <td>{{ $resident->account }}</td>
                        <td class="text-end">
                            <a href="{{ url('/addresidents/'.$resident->id.'/edit') }}" class="btn btn-sm btn-outline-primary">
                                <i class="bi bi-pencil-square"></i> 編輯
                            </a>
                            <form action="{{ url('/addresidents/'.$resident->id) }}" method="POST" class="d-inline" onsubmit="return confirm('確定刪除這位住戶嗎？');">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center text-muted">尚無住戶資料，請先新增。</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
...
<div class="corner-dock" aria-label="右下角功能列">
    ...
    <a class="dock-btn" href="{{ url('/addresidents') }}" style="text-decoration: none;"><i class="bi bi-person-video2"></i> 新增住戶</a>
    <a class="dock-btn is-active" href="{{ url('/residentmanagement') }}" style="text-decoration: none;"><i class="bi bi-people-fill"></i> 住戶清單</a>
</div>
