...
<section class="ba-panel" aria-label="裝置清單">
    ...
    <div class="panel-body">
        <form action="@if(isset($resident)) {{ url('/addresidents/' . $resident->id) }} @else {{ url('/addresidents') }} @endif" method="POST">
            @csrf
            @if(isset($resident))
                @method('PUT')
            @endif
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    <label for="name" class="form-label">住戶名稱 <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                    <label for="floor_id" class="form-label">樓層 <span class="text-danger">*</span></label>
                    <select class="form-select" id="floor_id" name="floor_id" required>
                        <option value="">請選擇樓層</option>
                        @foreach($floors as $floor)
                            <option value="{{ $floor->id }}" @if(old('floor_id', $resident->floor_id ?? '') == $floor->id) selected @endif>
                                {{ $floor->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <label for="door_number" class="form-label">門號 <span class="text-danger">*</span></label>
                    <input type="text" class="form-control" id="door_number" name="door_number" value="{{ old('door_number', $resident->door_number ?? '') }}" placeholder="例如：101, 202" required>
                    @error('door_number')
                        <div class="text-danger small mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    <label for="contact_phone" class="form-label">聯絡電話 <span class="text-danger">*</span></label>
                    <input type="text" class="form-control" id="contact_phone" name="contact_phone" value="{{ old('contact_phone', $resident->contact_phone ?? '') }}" placeholder="例如：0912-345-678" required>
                    @error('contact_phone')
                        <div class="text-danger small mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control" id="account" name="account" value="{{ old('account', $resident->account ?? '') }}" required>
                    @error('account')
                        <div class="text-danger small mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    <label for="password" class="form-label">密碼 @if(isset($resident)) <span class="text-muted small">(留空則不修改)</span> @else <span class="text-danger">*</span> @endif</label>
                    <input type="password" class="form-control" id="password" name="password" @if(!isset($resident)) required @endif>
                    @error('password')
                        <div class="text-danger small mt-1">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mt-4 d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    ...
                    @if(isset($resident)) 更新住戶 @else 新增住戶 @endif
                </button>
                <a href="{{ url('/residentmanagement') }}" class="btn btn-secondary">
...
<nav class="quick-toolbar" aria-label="快速工具列">
    <a class="floor-btn d-flex flex-column align-items-center justify-content-center gap-1 is-active" href="{{ url('/residentmanagement') }}" >
        <i class="bi bi-house-gear-fill fs-4" aria-hidden="true"></i><span>住戶管理</span>
    </a>
    <a class="floor-btn d-flex flex-column align-items-center justify-content-center gap-1" href="{{ url('/organizationalmanagement') }}">
...
<div class="corner-dock" aria-label="右下角功能列">
    <a class="dock-btn " href="{{ url('/addfloor') }}" style="text-decoration: none;"><i class="bi bi-stack"></i> 新增樓層</a>
    <a class="dock-btn " href="{{ url('/floors') }}" style="text-decoration: none;"><i class="bi bi-building"></i> 樓層清單</a>
    <a class="dock-btn is-active" href="{{ url('/addresidents') }}" style="text-decoration: none;"><i class="bi bi-person-video2"></i> 新增住戶</a>
    <a class="dock-btn " href="{{ url('/residentmanagement') }}" style="text-decoration: none;"><i class="bi bi-people-fill"></i> 住戶清單</a>
</div>
