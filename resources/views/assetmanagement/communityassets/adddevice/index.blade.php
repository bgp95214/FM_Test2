...
<section class="ba-panel" aria-label="裝置清單">
    ...
    <div class="panel-body">
        <div class="row mb-3">
            <div class="col">
                <h5 class="fw-semibold mb-0">新增設備</h5>
                <small class="text-muted">填寫設備資料並儲存</small>
            </div>
        </div>

        <div class="card shadow-sm" style="border: var(--glass-border);">
            <div class="card-body">
               
                <form action="@if(isset($device)) {{ route('adddevice.update', $device->id) }} @else {{ route('adddevice.store') }} @endif" method="POST" class="row g-4">
                @csrf
                @if(isset($device))
                    @method('PUT')
                @endif
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" value="{{ old('name', $device->name ?? '') }}" class="form-control" placeholder="請輸入設備名稱" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="asset_id" class="form-select" required>
                                ...
                                @foreach(($assets ?? []) as $asset)
                                    <option value="{{ $asset->id }}" @if(old('asset_id', $device->asset_id ?? '') == $asset->id) selected @endif>{{ $asset->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-select" required>
                            <option value="" disabled @if(!old('status', $device->status ?? '')) selected @endif>請選擇</option>
                            <option value="normal" @if(old('status', $device->status ?? '') === 'normal') selected @endif>正常</option>
                            <option value="maintenance" @if(old('status', $device->status ?? '') === 'maintenance') selected @endif>維修中</option>
                        </select>
                    </div>

                    <div class="col-12">
                        <label class="form-label">內容</label>
                        <textarea name="content" class="form-control" rows="3" placeholder="設備描述或備註">{{ old('content', $device->content ?? '') }}</textarea>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">購買日期</label>
                        <input type="date" name="purchase_date" value="{{ old('purchase_date', isset($device->purchase_date) ? $device->purchase_date->format('Y-m-d') : '') }}" class="form-control" required>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">保固期限（年）</label>
                        <input type="number" name="warranty_end" value="{{ old('warranty_end', $device->warranty_end ?? '') }}" class="form-control" min="0" placeholder="請輸入年數" required>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">維修廠商</label>
                        <select name="vendor_id" class="form-select" required>
                            <option value="" disabled @if(!old('vendor_id', $device->vendor_id ?? '')) selected @endif>請選擇</option>
                            @foreach(($vendors ?? []) as $vendor)
                                <option value="{{ $vendor->id }}" @if(old('vendor_id', $device->vendor_id ?? '') == $vendor->id) selected @endif>{{ $vendor->name }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="col-12 d-flex justify-content-end gap-2">
                        <a href="{{ url('/communityassets') }}" class="btn btn-light">取消</a>
                        <button type="submit" class="btn btn-primary" style="background:var(--clr-primary); border-color:var(--clr-primary);">儲存</button>
                    </div>
                </form>
            </div>
        </div>
...
<nav class="quick-toolbar" aria-label="快速工具列">
    <a class="floor-btn d-flex flex-column align-items-center justify-content-center gap-1 is-active" href="{{ url('/communityassets') }}" >
        <i class="bi bi-clipboard2-pulse fs-4" aria-hidden="true"></i><span>社區資產</span>
    </a>
    <a class="floor-btn d-flex flex-column align-items-center justify-content-center gap-1 " href="{{ url('/carmanagement') }}">
        <i class="bi bi-car-front-fill fs-4" aria-hidden="true"></i><span>公務車</span>
    </a>
</nav>

<!-- 右側工具鍵欄 -->
<div class="corner-dock" aria-label="右下角功能列">
    <a class="dock-btn " href="{{ url('/addassets') }}" style="text-decoration: none;"><i class="bi bi-bookmark-plus-fill"></i> 新增分類</a>
    <a class="dock-btn " href="{{ url('/assetslist') }}" style="text-decoration: none;"><i class="bi bi-bookmark-dash-fill"></i> 分類列表</a>
    <a class="dock-btn is-active" href="{{ url('/adddevice') }}" style="text-decoration: none;"><i class="bi bi-clipboard2-plus"></i> 新增設備</a>
    <a class="dock-btn " href="{{ url('/communityassets') }}" style="text-decoration: none;"><i class="bi bi-clipboard2-pulse"></i> 設備列表</a>
