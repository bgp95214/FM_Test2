...
<section class="ba-panel" aria-label="裝置清單">
    ...
    <div class="panel-body">
        ...
        <form action="@if(isset($assets)) {{ url('/addassets/'.$assets->id) }} @else {{ url('/addassets') }} @endif" method="POST">
            @csrf
            @if(isset($assets))
                @method('PUT')
            @endif
        ...
        <div class="container-fluid">
            <div class="row justify-content-center">
                <div class="col-lg-8">
                    ...
                    <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $assets->name ?? '') }}" placeholder="例如：燈光、水管、電力" required>
                    @error('name')
                    ...
                    <div class="mt-4 d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            ...
                            @if(isset($assets)) 更新分類 @else 新增分類 @endif
                        </button>
                        <a href="{{ url('/assetslist') }}" class="btn btn-secondary">
...
<div class="corner-dock" aria-label="右下角功能列">
    <a class="dock-btn is-active" href="{{ url('/addassets') }}" style="text-decoration: none;">
        <i class="bi bi-{{ isset($assets) ? 'pencil-square' : 'bi bi-bookmark-plus-fill' }}"></i> 
        {{ isset($assets) ? '編輯分類' : '新增分類' }}
    </a>
