...
<section class="ba-panel" aria-label="裝置清單">
    ...
    <div class="panel-body">
        <div class="row mb-3">
            <div class="col">
                <h5 class="fw-semibold mb-0">設備列表</h5>
                <small class="text-muted">社區資產設備清單</small>
            </div>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover">
                    <thead style="background: #f8f9fa;">
                        <tr>
                            <th class="sortable">設備名稱</th>
                            <th class="sortable">分類</th>
                            <th class="sortable">狀態</th>
                            <th class="sortable">購買日期</th>
                            <th class="sortable">已用/剩餘</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($devices as $device)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $purchase = $device->purchase_date;
                                        $warrantyYears = (float)($device->warranty_end ?? 0);
                                        $usedDays = $purchase ? $purchase->diffInDays(\Carbon\Carbon::now()) : 0;
                                        $usedYears = $usedDays / 365.25; // 以天數換算年，含閏年近似
                                        $remainingYears = max(0, $warrantyYears - $usedYears);
                                    @endphp
                                    已用 {{ number_format($usedYears, 1) }} 年 / 剩餘 {{ number_format($remainingYears, 1) }} 年
                                </td>
                                
                                <td class="text-end ">
                                        <button type="button" class="btn btn-sm btn-outline-info" data-bs-toggle="modal" data-bs-target="#contentModal" data-title="{{ $device->name }}" data-content="{{ $device->content ?? '無內容' }}">
                                            <i class="bi bi-card-text"></i> 內容
                                        </button>
                                        <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#vendorModal" data-vendor-name="{{ $device->vendor->name ?? '未指定廠商' }}" data-contact="{{ $device->vendor->contact_person ?? '無聯絡人資料' }}" data-phone="{{ $device->vendor->phone ?? '無電話資料' }}" data-email="{{ $device->vendor->email ?? '無Email資料' }}" data-service="{{ $device->vendor->service_item ?? '無服務項目資料' }}">
                                            <i class="bi bi-person-lines-fill"></i> 廠商
                                        </button>
                                        <a href="{{ url('/adddevice/'.$device->id.'/edit') }}" class="btn btn-sm btn-outline-primary">
                                            <i class="bi bi-pencil-square"></i> 編輯
                                        </a>
                                        <form action="{{ url('/adddevice/'.$device->id) }}" method="POST" class="d-inline" onsubmit="return confirm('確定刪除這個設備嗎？');">
                                            @csrf
                                            @method('DELETE')
        ...
        @else
            <div class="alert alert-info text-center" role="alert">
                <i class="bi bi-info-circle"></i> 目前沒有設備資料
            </div>
        @endif

        <!-- 內容檢視 Modal -->
        <div class="modal fade" id="contentModal" tabindex="-1" aria-labelledby="contentModalLabel" aria-hidden="true" data-bs-backdrop="false">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="contentModalLabel">設備內容</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body" id="contentModalBody" style="white-space: pre-wrap;"></div>
...
<div class="corner-dock" aria-label="右下角功能列">
    ...
    <a class="dock-btn " href="{{ url('/adddevice') }}" style="text-decoration: none;"><i class="bi bi-clipboard2-plus"></i> 新增設備</a>
    <a class="dock-btn is-active" href="{{ url('/communityassets') }}" style="text-decoration: none;"><i class="bi bi-clipboard2-pulse"></i> 設備列表</a>
...
<script>
    ...
    if (contentModal) {
        contentModal.addEventListener('show.bs.modal', event => {
            ...
            const title = button?.getAttribute('data-title') || '設備內容';
            const content = button?.getAttribute('data-content') || '無內容';
