...
<section class="ba-panel" aria-label="裝置清單">
    ...
    <div class="panel-body">
        <div class="row mb-3">
            <div class="col">
                <h5 class="fw-semibold mb-0">新增公務車</h5>
                <small class="text-muted">填寫公務車基本資料</small>
            </div>
        </div>

        <form action="{{ isset($car) ? route('addcar.update', $car->id) : route('addcar.store') }}" method="POST">
            @csrf
            @if(isset($car))
                @method('PUT')
            @endif
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <label for="brand" class="form-label">車子品牌</label>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" class="form-control" id="purchase_date" name="purchase_date" value="{{ old('purchase_date', isset($car) && $car->purchase_date ? $car->purchase_date->format('Y-m-d') : '') }}" required>
                </div>
                <div class="col-md-6">
                    <label for="maintenance_method" class="form-label">保養方式</label>
                    <textarea class="form-control" id="maintenance_method" name="maintenance_method" rows="4" required>{{ old('maintenance_method', $car->maintenance_method ?? '') }}</textarea>
                </div>
                <div class="col-md-6">
                    <label for="status" class="form-label">狀態</label>
                    <select class="form-select" id="status" name="status" required>
                        @php $selectedStatus = old('status', $car->status ?? 'normal'); @endphp
                        <option value="normal" {{ $selectedStatus === 'normal' ? 'selected' : '' }}>正常</option>
                        <option value="maintenance" {{ $selectedStatus === 'maintenance' ? 'selected' : '' }}>維修中</option>
                    </select>
                </div>
                <div class="col-md-6">
                    <label for="vendor_id" class="form-label">維修廠商</label>
                    <select class="form-select" id="vendor_id" name="vendor_id">
                        <option value="">請選擇廠商</option>
                        @foreach($vendors as $vendor)
                            <option value="{{ $vendor->id }}" {{ (string)$vendor->id === (string)old('vendor_id', $car->vendor_id ?? '') ? 'selected' : '' }}>
                                {{ $vendor->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mt-4">
                <button type="submit" class="btn btn-primary">{{ isset($car) ? '更新' : '儲存' }}</button>
                <a href="{{ url('/carmanagement') }}" class="btn btn-secondary ms-2">返回列表</a>
            </div>
        </form>
    </div>
</section>
...
<div class="corner-dock" aria-label="右下角功能列">
    <a class="dock-btn is-active" href="{{ url('/addcar') }}" style="text-decoration: none;"><i class="bi bi-patch-plus"></i> 新增車輛</a>
    <a class="dock-btn " href="{{ url('/carmanagement') }}" style="text-decoration: none;"><i class="bi bi-car-front-fill"></i> 車輛列表</a>
</div>
